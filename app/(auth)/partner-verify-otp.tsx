...
export default function PartnerVerifyOTP() {
  const { phoneNumber, organization } = useLocalSearchParams<{
    phoneNumber: string;
    organization?: string;
  }>();
  ...
  const orgValue =
    typeof organization === "string"
      ? organization
      : organization?.[0] ?? Organization.SKYLINE;
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setIsLoading(true);
      const response = await partnerLogin(phoneNumber, code, orgValue);
      ...
      console.log("Partner verification error:", JSON.stringify(error));
  ...
  const handleResendCode = async () => {
    try {
      await sendPartnerOtp(phoneNumber);
